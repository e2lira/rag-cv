...
| CA-3 | `EMBEDDING_DIM=1536` existe en `Settings` y el DDL declara `VECTOR(1536)`. Que el **proceso real aborte** al desincronizarse lo entrega RFC-0021 CA-1/A-1: la dimensión es una de las cinco comprobaciones de RFC-0006 §7, y el esqueleto de RFC-0011 no puede cablearlas sin contradecir su CA-4 | *job* `integration-linux` de `python-tests.yml` + `test_startup_checks.py::test_dim_mismatch` | **Este RFC** el valor; **RFC-0021** el aborto real |
